 ...
 * which has Rectangle and Triangle. And each shape has two color variants red and yellow
 * so you extended and Created RedRectangleShape and YellowRentangleShape and same triangle.
 * So we have now this 4 different variants of shape , consider you decided to expand for Blue color and Circle shape.
 * So your classes keep getting increased.
 *
 ...
 * 1. Abstraction : MusicPlayer has a video processor
 * 2. RefinedAbstraction : Which is our HomeMusicPlayer and PortableMusicPlayer
 * 3. Implementor : AudioOutput
 * 4. Concrete Implementor : Headphone and Speaker.
 *
 * we can add more musicplayer and audiooutput in future.
 */
public class Main {
    public static void main(String[] args) {
        // Using headphones with a portable music player
        AudioOutput headphones = new Headphones();
        MusicPlayer portablePlayer = new PortableMusicPlayer(headphones);
        portablePlayer.playMusic();
        portablePlayer.stopMusic();

        System.out.println("-------------");

        // Using speakers with a home music player
        AudioOutput speakers = new Speaker();
        MusicPlayer homePlayer = new HomeMusicPlayer(speakers);
        homePlayer.playMusic();
        homePlayer.stopMusic();
    }
}
